...
def subject_card(name, code, section, stats=None, footer_callback=None):
    ...
    if stats:
        ...
        for icon, label, value in stats:
            # Added "color: #000000;" right here to force the text to black
            html+= f'<div style="background: #EB459E10; color: #000000; padding:5px 12px; border-radius:12px; font-size:0.9rem">{icon} <b>{value}</b> {label} </div>'
